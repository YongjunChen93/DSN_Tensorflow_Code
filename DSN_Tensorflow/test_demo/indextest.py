...
print("shape_coord",sess.run(tf.shape(Coordinate)))
Temp = tf.unstack(Coordinate)
print("shape_Temp",sess.run(tf.shape(Temp)))
print("shape_",sess.run(tf.shape(Temp)[0]))
B = tf.unstack(tf.reshape(Coordinate,[-1,len(Temp)]))
print("shapeB",sess.run(tf.shape(B)))
Batch_index = []
...
#q= tf.sparse_to_dense(sparse_indices = [[1,2],[2,1]], output_shape = [3,3], 
#    sparse_values = [2,3], default_value = 1)
#print(sess.run(tf.shape(q)))
#print(sess.run(q))
