...
def makeT(cp, Row_P_N, Column_P_N):
	#cp = tf.tanh(cp)
	N_f = 1
	#shape_cp = sess.run(tf.shape(cp))
	#print("cpshape",shape_cp)
	#cp': K x 2 control point
	...
	#print("R",sess.run(R))
	#R = tf.reshape(R,(-1,tf.multiply(Row_P_N,Column_P_N),tf.multiply(Row_P_N,Column_P_N)))
	...
	#print("Deltas_inv_shape",sess.run(tf.shape(Deltas_inv)))
	#print("Deltas_inv",sess.run(Deltas_inv))
	Deltas_inv_f = tf.tile(Deltas_inv,[1,1,1])	
	...
	#print("cp trans",sess.run(tf.shape(cp_trans)))
	
	zeros_f_In = tf.zeros([N_f,3,2],tf.float32)
	#print("zeros_f_In",sess.run(tf.shape(zeros_f_In)))	
	
	cp = tf.concat([cp_trans,zeros_f_In],1)
	#print("cp_final",sess.run(tf.shape(cp)))
	#print("cp_finalvalue",sess.run(cp))
	#cp = tf.reshape(cp,[-1])
	#print("cp_final",sess.run(tf.shape(cp)))
	
	#T = tf.transpose(tf.concat([cp_trans,zeros_f_In],1),[0,2,1])
...
def meshgrid(num_batch,height, width,Column_controlP_number,Row_controlP_number):
	...
	print(sess.run(tf.shape(x_t)))
	y_t = tf.matmul(tf.expand_dims(tf.linspace(-1.0, 1.0, height), 1),tf.ones(shape=tf.stack([1, width])))
	print(sess.run(tf.shape(y_t)))
	x_t_flat = tf.reshape(x_t, (1, -1))
	print(sess.run(tf.shape(x_t_flat)))
	print("x_t_flats",sess.run(x_t_flat))
	y_t_flat = tf.reshape(y_t, (1, -1))
	print(sess.run(tf.shape(y_t_flat)))

	px,py = tf.stack([x_t_flat],axis=2),tf.stack([y_t_flat],axis=2)
	print(sess.run(tf.shape(px)))
	print(sess.run(tf.shape(py)))
